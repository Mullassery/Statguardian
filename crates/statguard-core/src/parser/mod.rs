        ...
        match section.as_rule() {
            Rule::section => {
                ...
                match inner.as_rule() {
                    ...
                    Rule::quality_section => contract.quality_rules = parse_quality(inner)?,
...
fn parse_quality(pair: pest::iterators::Pair<Rule>) -> CoreResult<Vec<QualityRule>> {
    let mut rules = Vec::new();
    for r in pair.into_inner() {
        if r.as_rule() == Rule::quality_rule {
            rules.push(parse_quality_rule(r)?);
        }
    }
    Ok(rules)
}
...
"#;
    ...
    fn test_field_types_and_constraints() {
        ...
        assert!(schema[0].constraints.contains(&Constraint::Unique));
    ...
    fn test_severity_prefix() {
        ...
        assert_eq!(quality[0].severity, Severity::Error); // default
        assert_eq!(quality[1].severity, Severity::Warning);

        let anomalies = &contracts[0].anomaly_rules;
        assert_eq!(anomalies[0].severity, Severity::Error);
        assert_eq!(anomalies[1].severity, Severity::Blocking);
    }
}
